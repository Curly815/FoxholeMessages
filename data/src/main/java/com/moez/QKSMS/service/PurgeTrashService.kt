...
class PurgeTrashService : JobService() {

    companion object {
        private const val JOB_ID = 8120237

        @SuppressLint("MissingPermission") // Added in [presentation]'s AndroidManifest.xml
        fun scheduleJob(context: Context) {
            Timber.i("Scheduling job")
            val serviceComponent = ComponentName(context, PurgeTrashService::class.java)
    ...
    @Inject lateinit var purgeTrash: PurgeTrash
    ...
    override fun onStartJob(params: JobParameters?): Boolean {
        ...
        disposables += purgeTrash
        purgeTrash.execute(Unit) {
